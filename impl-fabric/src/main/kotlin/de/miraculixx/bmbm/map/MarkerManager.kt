...
import de.miraculixx.bmbm.consoleAudience
import de.miraculixx.bmbm.prefix
import de.miraculixx.bmbm.utils.cError
...
object MarkerManager {
    ...
    fun addMarker(marker: POIMarker, worldName: String, playerUUID: UUID) {
        ...
        if (markerSet == null) {
            consoleAudience.sendMessage(prefix + cmp("Failed to apply marker to $worldName! Please reload BlueMap after creating new worlds", cError))
            return
        }
    ...
    fun loadAllMarker(blueMapAPI: BlueMapAPI) {
        ...
        BlueMapAPI.getInstance().ifPresent {
            it.worlds.forEach { world ->
                val worldName = world.id
                val markerFile = File("${folder.path}/${worldName}.json")
                val set = if (markerFile.exists()) {
                    consoleAudience.sendMessage(prefix + cmp("Found markers for world '$worldName' - Loading ${markerFile.length() / 1000.0}kb"))
                    try {
                        gson.fromJson(markerFile.readText(), MarkerSet::class.java)
                    } catch (e: JsonSyntaxException) {
                        consoleAudience.sendMessage(prefix + cmp("Marker file for world $worldName is invalid! Skipping it..."))
                        return@forEach
                    }.apply {
                ...
                world.maps.forEach { map ->
                    map.markerSets["BANNER_MARKER_${world.id}"] = set
                }
            }
        }
